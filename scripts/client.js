...
async function startGame(){
    try {
        ...
        if (anims.idle) anims.idle.start(true); // Start the idle animation initially

        // Set the assetsLoaded flag to true once loading is complete
        assetsLoaded = true;
    } catch (error) {
...
function createRemotePlayer(playerId, position) {
    ...
    // Set the cloned model's visibility to true when needed
    clonedModel.isVisible = true;

    // Assign the cloned model to the remotePlayers object
    remotePlayers[playerId] = {
        model: clonedModel,
        animations: playerModel.animations, // Reuse the animations from the original model
